...
namespace GridMvc
{
    ...
    public abstract class GridBase<T> where T : class
    {
        ...
        /*Custom Modification By Arsalan :: 07/08/2016*/
        private Func<T, string> _rowAtrContraint;

        public void SetRowAtrContraint(Func<T, string> contraint)
        {
            _rowAtrContraint = contraint;
        }

        public string GetRowAtr(object item)
        {
            if (_rowAtrContraint == null)
                return string.Empty;
            var typed = item as T;
            if (typed == null)
                throw new InvalidCastException(string.Format("The item must be of type '{0}'", typeof(T).FullName));
            return _rowAtrContraint(typed);
        }
